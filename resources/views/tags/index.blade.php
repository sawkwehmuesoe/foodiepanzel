                        ...
                        <table id="mytable" class="table rounded shadow">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="p-4 text-muted text-center">{{$tag->status_id}}</td>
                                    <td class="p-4 text-muted text-center">{{$tag->created_at->format('d M Y h:i:s')}}</td>

                                    <td class="text-center">
                                        <a href="javascript:void(0);" class="text-info editform" data-bs-toggle="modal" data-bs-target="#editmodal" data-id={{$tag->id}} data-name={{$tag->name}}><i class="fas fa-pen"></i></a>
                                        <a href="#" class="text-danger ms-2 delete-btns" data-idx="{{$tag->name}}"><i class="fas fa-trash-alt"></i></a>
                                    </td>
                                    <form id="formdelete-{{$tag->name}}" action="{{route('tags.destroy',$tag->id)}}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </tr>
